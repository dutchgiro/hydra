...
var _ = Describe("EtcdSupervisor", func() {
	...
	BeforeEach(func() {
		...
		etcdSupervisor = NewEtcdSupervisor(hydraConfig, mockEtcdClient)
	...
	Context("when the node starts", func() {
		It("should try setting its state", func() {
			// etcdSupervisor.EtcdClient = mockEtcdClient
			// mockEtcdClient.EXPECT().Set(gomock.Eq(stateKeyPath), gomock.Eq(StateAvailable), gomock.Eq(etcdSupervisor.GetStateTTL())).Times(1)
			mockEtcdClient.EXPECT().Set(gomock.Any(), gomock.Any(), gomock.Any()).Times(2)
	...
	Context("when the node can not publish the node state", func() {
		...
		// 			gomock.Any(), gomock.Any()).Return(nil, errors.New("Write operation impossible")).Times(int(etcdSupervisor.GetNumOfSetStateRetries()))
